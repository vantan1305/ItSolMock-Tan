...
public class AuthRestAPIs {
    ...
    public ResponseEntity authenticateUser(@Valid @RequestBody LoginForm loginRequest) {
        ...
        Users currentUser = userDetailsService.findByUsername(loginRequest.getUserName ()).get();
//            them moi
        List<String> roles = userDetails.getAuthorities ().stream ()
                .map (item -> item.getAuthority ())
                .collect(Collectors.toList());

        return ResponseEntity.ok(new JwtResponse(jwt, userDetails.getUsername (), userDetails.getAuthorities(), roles, currentUser.getId ()));
    }
